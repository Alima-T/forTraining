...
public abstract class Animal { //Animal  не является сущностью, а является концепцией, поэтому, чтобы запретитьсоздание объектов данного класса, мы добавляем abstract в декларации класса,
    //теперь java видит, что это не конкретный класс, от которого должны быть созданы объекты, а концепция, которая включает в себя некоторые признаки, присущие всем животным
    //от него нужно наследоваться, и реализовать абстрактные методы из абстрактного класса, не реализовав объекты мы не сможем создавать объекты в классе-наследнике, которые уже станут сущностями.
    public void eat (){
...
/* abstract используется когда мы хотим создать иерархию классов, но класс-родитель не является самостоятельн обязательно реализовать абстрактные методы, если они есть в абстрактном класса-родителе
...
Концепция Абстрактного Типа Данных
        ...
        Что же означает слово “абстрактный”? В первую очередь понятие “абстрактность” означет сосредоточение внимания на чем-то важном и,
        при этом, нам нужно отвлечься от неважных, на данный момент, деталей. Определение абстрактности хорошо раскрыто в книге Гради Буча (“Grady Booch”). Звучит определение так:
